...
hatchling picks it up via ``packages = ["src/sim_plugin_openfoam"]``
...
def test_wheel_contains_skills(tmp_path: Path) -> None:
    ...
    wheels = list(out_dir.glob("sim_plugin_openfoam-*.whl"))
    ...
    required = {
        "sim_plugin_openfoam/__init__.py",
        "sim_plugin_openfoam/driver.py",
        "sim_plugin_openfoam/_skills/openfoam/SKILL.md",
    }
